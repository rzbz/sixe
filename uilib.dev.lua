...
local NAME = _G.SIXE_NAME or "aw"
local GAME_NAME = _G.SIXE_GAME_NAME or "ew"

local configFile = `./{NAME}/{GAME_NAME}.json`

pcall(function()
  if not isfolder(NAME) then
    makefolder(NAME)
  end
	userinfo = HttpService:JSONDecode(readfile(configFile));
end)
...
local function SaveInfo()
	...
	userinfo["user"] = user
	userinfo["tag"] = tag
	writefile(configFile, HttpService:JSONEncode(userinfo));
end
...
Discord.DisplayOrder = 99

local function offsetToScale(guiObject, useViewport, parent)
  local newSize
  ...
  if useViewport then
    ...
    newSize = UDim2.new(
      size.X.Offset / ViewPortSize.X, 0,
      size.Y.Offset / ViewPortSize.Y, 0
    )
  else
    parent = parent or guiObject.Parent
    if not parent then return end
    local size = parent.AbsoluteSize
    newSize = UDim2.new(
      guiObject.Size.X.Offset / size.X, 0,
      guiObject.Size.Y.Offset / size.Y, 0
    )
  end
...
function DiscordLib:Window(text)
  ...
  local MainFrameOffsetSize = {
	...
	MainFrame.Size = UDim2.new(0, MainFrameOffsetSize.x, 0, MainFrameOffsetSize.y)
  offsetToScale(MainFrame, true)
	...
	TopFrame.Size = UDim2.new(0, MainFrameOffsetSize.x, 0, 22)
	...
	TopFrameHolder.Size = UDim2.new(0, MainFrameOffsetSize.x, 0, 22)
	...
	MinimizeBtn.Position = UDim2.new(0.959063113, 0, -0.0169996787, 0)
	...
	Userpad.Position = UDim2.new(0.106243297, 0, 13.1, 0)
	Userpad.Size = UDim2.new(0, 179, 0, 43)
  offsetToScale(Userpad)
	...
	UserName.Text = "thisismykey" -- user
	UserTag.Text = "til 14 hrs" -- "#" .. tag
	...
	ServersHoldFrame.Size = UDim2.new(0, 71, 0, MainFrameOffsetSize.y)
	...
	ServersHold.Size = UDim2.new(0, 71, 0, 303)
  ...
  local function toggleWindow()
	...
	SettingsFrame.Size = UDim2.new(0, MainFrameOffsetSize.x, 0, MainFrameOffsetSize.y)
	SettingsFrame.Visible = false
  offsetToScale(SettingsFrame)
	...
	Settings.Size = UDim2.new(0, MainFrameOffsetSize.x, 0, 375)
	...
	SettingsOpenBtn.MouseButton1Click:Connect(function ()
		settingsopened = true
			...
			SettingsHolder:TweenSize(UDim2.new(0, MainFrameOffsetSize.x, 0, 375), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .3, true)
	...
	function DiscordLib:Notification(titletext, desctext, btntext)
		...
		NotificationHolderMain.Size = UDim2.new(0, MainFrameOffsetSize.x, 0, 374)
    ...
    -- background overlay
    -- NotificationHolderMain.MouseButton1Click:Connect(closeNotif)
		
		AlrightBtn.MouseButton1Click:Connect(closeNotif)
	...
	local ServerHold = {}
  function ServerHold:showWindow()
    showWindow()
  end
  function ServerHold:hideWindow()
    hideWindow()
  end
  function ServerHold:toggleWindow()
    toggleWindow()
  end
  function ServerHold:Destroy()
    hideWindow()
    Discord:Destroy()
  end
	function ServerHold:Server(text, img, isEnabled, onServerClick)
		local fc = false
		...
		ServerWhiteFrame.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
		ServerWhiteFrame.Position = UDim2.new(-0.347378343, 0, 0.502659559, 0)
		ServerWhiteFrame.Size = UDim2.new(0, 11, 0, 10)
    -- ? notif white icon next to server btn
    ServerWhiteFrame.Visible = isEnabled or false
    ...
    offsetToScale(ServerContentFrame)
    ...
    offsetToScale(ChannelContentFrame)
		...
		ServerChannelHolder.Size = UDim2.new(0, 179, 0, 215)
    ...
    if true then
      ...
      Server.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
          ...
          while holding do
            if tick() - startTime >= holdTime then
              print("✅ Long press triggered (touch or mouse)!")
              DiscordLib:Notification("Global Config's settings", "Duplicate config", "Okay")
              break
            end
            task.wait(0.1)
          end
      ...
      Server.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 
        or input.UserInputType == Enum.UserInputType.Touch then
          holding = false
        end
      ...
      ServerIco.Image = img
		...
		local ChannelHold = {}
    function ChannelHold:Category(text)
      local CategoryBtn = Instance.new("TextButton")
      local CategoryBtnTitle = Instance.new("TextLabel")

      local isOpen = false
      local openArrow = "˅"
      local closeArrow = ">"

      CategoryBtn.Name = text .. "CategoryBtn"
      CategoryBtn.Parent = ServerChannelHolder
      CategoryBtn.BackgroundColor3 = Color3.fromRGB(47, 49, 54)
      CategoryBtn.BorderSizePixel = 0
      -- CategoryBtn.Position = UDim2.new(0.24118948, 0, 0.578947365, 0)
      CategoryBtn.Position = UDim2.new(0.05, 0, 0.578947365, 0)
      CategoryBtn.Size = UDim2.new(0, 160, 0, 25)
      CategoryBtn.AutoButtonColor = false
      CategoryBtn.Font = Enum.Font.SourceSans
      CategoryBtn.Text = ""
      CategoryBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
      CategoryBtn.TextSize = 14.000
      CategoryBtn:SetAttribute("CategoryBtn", true)
      
			CategoryBtnTitle.Name = "CategoryBtnTitle"
			CategoryBtnTitle.Parent = CategoryBtn
			CategoryBtnTitle.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			CategoryBtnTitle.BackgroundTransparency = 1.000
			CategoryBtnTitle.BorderSizePixel = 0
			-- CategoryBtnTitle.Position = UDim2.new(0.173747092, 0, -0.166666672, 0)
			CategoryBtnTitle.Position = UDim2.new(0.05, 0, -0.166666672, 0)
			CategoryBtnTitle.Size = UDim2.new(0, 95, 0, 39)
			CategoryBtnTitle.Font = Enum.Font.GothamBold
			CategoryBtnTitle.Text = string.upper(closeArrow.." "..text)
			CategoryBtnTitle.TextColor3 = Color3.fromRGB(114, 118, 125)
			CategoryBtnTitle.TextSize = 10.000
			CategoryBtnTitle.TextXAlignment = Enum.TextXAlignment.Left

      CategoryBtn.MouseEnter:Connect(function()
				CategoryBtn.BackgroundColor3 = Color3.fromRGB(52,55,60)
			end)
      ...
      local channelBtns = {}
        ...
        if isOpen then
          -- CategoryBtnArrow.Text = closeArrow
          CategoryBtnTitle.Text = string.upper(closeArrow.." "..text)
          for _, btn in ipairs(channelBtns) do btn.Visible = false end
          isOpen = false
        else
          -- CategoryBtnArrow.Text = openArrow
          CategoryBtnTitle.Text = string.upper(openArrow.." "..text)
          for _, btn in ipairs(channelBtns) do btn.Visible = true end
          isOpen = true
        end
      ...
      local CategoryHold = {}
      function CategoryHold:Channel(text)
        return ChannelHold:Channel(text, function(channelBtn)
          channelBtn.Visible = false
          table.insert(channelBtns, channelBtn)
        end)
      end
      function CategoryHold:CategoryEnd()
        local CategoryEndGap = Instance.new("TextLabel")
        CategoryEndGap.Name = "CategoryEndGap"
        CategoryEndGap.Parent = ServerChannelHolder
        CategoryEndGap.BackgroundTransparency = 1
        CategoryEndGap.TextTransparency = 1
        CategoryEndGap.BorderSizePixel = 0
        CategoryEndGap.Position = UDim2.new(0.173747092, 0, -0.166666672, 0)
        CategoryEndGap.Size = UDim2.new(0, 95, 0, 5)
        CategoryEndGap.Text = ""
        CategoryEndGap.TextSize = 3.000
      end

      return CategoryHold
    end
		function ChannelHold:Channel(text, onCreateCb)
			...
			ChannelBtn.MouseButton1Click:Connect(function()
				...
				for i, v in next, ServerChannelHolder:GetChildren() do
					if v.ClassName == "TextButton" and not v:GetAttribute("CategoryBtn") then
      ...
      if onCreateCb then
        onCreateCb(ChannelBtn)
			...
			function ChannelContent:Dropdown(text, list, callback)
				...
				DropdownFrameOutline.Size = UDim2.new(0, MainFrameOffsetSize.y, 0, 36)
				...
				DropdownFrameMainOutline.Size = UDim2.new(0, MainFrameOffsetSize.y, 0, 81)
				...
				for i,v in next, list do
					...
					DropdownFrameMainOutline.Size = UDim2.new(0, MainFrameOffsetSize.y, 0, framesize + 10)
				end
				...
				function DropFunc:Clear()
					...
					DropdownFrameMainOutline.Size = UDim2.new(0, MainFrameOffsetSize.y, 0, 0)
					Dropdown.Size = UDim2.new(0, 403, 0, 73)
					DropdownFrameMain.Visible = false
					DropdownFrameMainOutline.Visible = false
					ChannelHolder.CanvasSize = UDim2.new(0,0,0,ChannelHolderLayout.AbsoluteContentSize.Y)
			...
			function ChannelContent:Textbox(text, placetext, disapper, callback)
				...
				TextboxFrameOutline.Size = UDim2.new(0, MainFrameOffsetSize.y, 0, 36)
...
local DiscordNotification = Instance.new("ScreenGui")
DiscordNotification.Name = "DiscordNotification"
DiscordNotification.Parent = game.CoreGui
DiscordNotification.ZIndexBehavior = Enum.ZIndexBehavior.Global
DiscordNotification.DisplayOrder = 99
DiscordNotification.IgnoreGuiInset = true

-- TODO: header
function DiscordLib:GlobalNotification(titletext, desctext, btntext)
  local NotificationOverlay = Instance.new("TextButton")
  local Notification = Instance.new("Frame")
  local NotificationCorner = Instance.new("UICorner")
  local UpperBar = Instance.new("Frame")
  local UpperBarCorner = Instance.new("UICorner")
  local UpperBarFrame = Instance.new("Frame")
  local UnderBar = Instance.new("Frame")
  local UnderBarCorner = Instance.new("UICorner")
  local UnderBarFrame = Instance.new("Frame")
  local Text1 = Instance.new("TextLabel")
  ...
  NotificationOverlay.Name = "NotificationOverlay"
  NotificationOverlay.Parent = DiscordNotification
  NotificationOverlay.BackgroundColor3 = Color3.fromRGB(22, 22, 22)
  NotificationOverlay.BackgroundTransparency = 1
  NotificationOverlay.BorderSizePixel = 0
  NotificationOverlay.AnchorPoint = Vector2.new(0.5, 0.5)
  NotificationOverlay.Position = UDim2.new(0.5, 0, 0.5, 0)
  NotificationOverlay.Size = UDim2.new(1, 0, 1, 0)
  NotificationOverlay.Text = ""
  NotificationOverlay.ZIndex = 0
  NotificationOverlay.AutoButtonColor = false
  TweenService:Create(
    NotificationOverlay,
    TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
    {BackgroundTransparency = 0.2}
  ...
  Notification.Name = "Notification"
  Notification.Parent = DiscordNotification
  Notification.AnchorPoint = Vector2.new(0.5, 0.5)
  Notification.BackgroundColor3 = Color3.fromRGB(54, 57, 63)
  Notification.ClipsDescendants = true
  Notification.Position = UDim2.new(0.524819076, 0, 0.469270051, 0)
  ...
  UpperBar.Parent = Notification
  UpperBar.BackgroundColor3 = Color3.fromRGB(47, 49, 54)
  UpperBar.Position = UDim2.new(0, 0, 0, 0)
  UpperBar.Size = UDim2.new(0, 346, 0, 10)

  UpperBarCorner.CornerRadius = UDim.new(0, 5)
  UpperBarCorner.Name = "UpperBarCorner"
  UpperBarCorner.Parent = UpperBar

  UpperBarFrame.Name = "UpperBarFrame"
  UpperBarFrame.Parent = UpperBar
  UpperBarFrame.BackgroundColor3 = Color3.fromRGB(47, 49, 54)
  UpperBarFrame.BorderSizePixel = 0
  UpperBarFrame.Position = UDim2.new(0, 0, 0, 0)
  UpperBarFrame.Size = UDim2.new(0, 346, 0, 40)
  ...
  local function closeNotif()
    ...
    NotificationOverlay:Destroy()
  end
  
  AlrightBtn.MouseButton1Click:Connect(closeNotif)

  NotificationOverlay.MouseButton1Click:Connect(closeNotif)
